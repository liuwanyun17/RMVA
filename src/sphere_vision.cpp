...
class TestNode : public rclcpp::Node {
 public:
  TestNode(string name) : Node(name) {
    ...
    RCLCPP_INFO(this->get_logger(), "Initializing sphere_vision_node");
    ...
    RCLCPP_INFO(this->get_logger(), "sphere_vision_node initialized successfully");
  }
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<TestNode>("sphere_vision_node");
  RCLCPP_INFO(node->get_logger(), "Starting sphere_vision_node");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
